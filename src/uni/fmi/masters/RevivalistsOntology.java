...
public class RevivalistsOntology {
	...
	final private String ontologyPath = "src/ontology/bulgarian_revivalists.owl";
	...
	public Set<OWLClass> getClassesThisIndividualBelongsTo(OWLIndividual indv) { // return a set of all classes this individual belongs to
		return reasoner.getTypes(indv.asOWLNamedIndividual(), false).getFlattened();
	}
	
	public boolean isIndividualBelongsTo(OWLIndividual indv, OWLClass cls) { // check if an individual belongs to some class (including subclasses)
	...
	public Set<OWLIndividual> getIndividualsByClass(OWLClass cls){ // get individuals belongs to some class (including from subclasses; without duplication)
	...
	public EntityActivity getActivityFromIndividual(OWLIndividual indv) {
		if (indv != null && isIndividualBelongsTo(indv, getOWLClassByName("Activity"))){
			...
			return new EntityActivity(activityName, activityStartedD, activityStartedM, activityStartedY, activityFinishedD, activityFinishedM, activityFinishedY);
		}
		return null;
	}
	...
	public EntityPerson getPersonFromIndividual(OWLIndividual indv) {
		if (indv != null && isIndividualBelongsTo(indv, getOWLClassByName("Person"))){
			...
			return new EntityPerson(
				getValueFromDataProperty(indv, "person_name_first"),
				getValueFromDataProperty(indv, "person_name_sur"),
				getValueFromDataProperty(indv, "person_name_family"),
				getValueFromDataProperty(indv, "person_name_alt"),
				personBornD, personBornM, personBornY, personDiedD, personDiedM, personDiedY
			);
		}
		return null;
	}
	...
	public EntityPlace getPlaceFromIndividual(OWLIndividual indv) {
		if (indv != null && isIndividualBelongsTo(indv, getOWLClassByName("Place"))){
			...
			///
			return new EntityPlace(
				getValueFromDataProperty(indv, "place_name"),
				getValueFromDataProperty(indv, "place_name_alt"),
	...
	public EntityWork getWorkFromIndividual(OWLIndividual indv) {
		if (indv != null && isIndividualBelongsTo(indv, getOWLClassByName("Work"))){
			...
			return new EntityWork(
				getValueFromDataProperty(indv, "work_name"),
				workCreatedY
			);
		}
		return null;		
	...
	public OWLIndividual getIndividualPersonByName(String name) {
		...
		for (OWLIndividual indv: indvs) {
			...
			if (name.equals(p.getNameFirst() + " " + p.getNameFamily()) || name.equals(p.getNameFirst() + " " + p.getNameSur() + " " + p.getNameFamily())
	...
	public OWLIndividual getIndividualPlaceByName(String name) {
		...
		for (OWLIndividual indv: indvs) {
			...
			if ((p.getName() != null && p.getName().equals(name)) || (p.getNameAlt() != null && p.getNameAlt().equals(name))) {
				return indv;
			}
		}
		return null;
